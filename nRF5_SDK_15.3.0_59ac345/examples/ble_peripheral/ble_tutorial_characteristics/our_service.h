...
typedef struct
{
    ...
    //is there to keep track of the current connection and don't really have anything to do with the attribute table handles. 
    uint16_t                    service_handle; /**< Handle of Our Service (as provided by the BLE stack). */
    // OUR_JOB: Step 2.D, Add handles for the characteristic attributes to our struct
    ble_gatts_char_handles_t    char_handles; //can hold 16-bit handles for the characteristic value, user descriptor, CCCD and SCCD
}ble_os_t;  //Our Service
